...
Class Mongo_db
{
    ...
    public function insert($collection = "", $insert = array())
    {
        ...
        try {
            ...
            $_id = $bulk->insert($insert);
            $this->db->executeBulkWrite($this->database . '.' . $collection, $bulk, $wc);
            if (isset($_id)) {
                return (string)($_id);
            } else {
                return (FALSE);
    ...
    public function like($field = "", $value = "", $flags = "i", $enable_start_wildcard = TRUE, $enable_end_wildcard = TRUE)
    {
        ...
        $regex = "$value";
        $this->wheres[$field] = new MongoDB\BSON\Regex($regex, $flags);
        return ($this);
    }
    ...
    public function count($collection = "")
    {
        ...
        $query = new MongoDB\Driver\Query($this->wheres, $options);
        $readPreference = new MongoDB\Driver\ReadPreference(MongoDB\Driver\ReadPreference::RP_PRIMARY);
        $count = $this->db->executeQuery($this->database . '.' . $collection, $query, $readPreference)->toArray();
        $this->_clear();
        return count($count);
    }
